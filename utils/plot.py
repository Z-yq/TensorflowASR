...
def plot_alignment(alignment, path, info=None, split_title=False, max_len=None):
    if max_len is not None:
        alignment = alignment[:max_len[0], :max_len[1]]
...
if __name__ == '__main__':
    with open('magicd_data_result.txt',encoding='utf-8') as f:
        data=f.readlines()
    r,i,s,n=0,0,0,0
    num=0
    for line in data:
        _,_,_,result=line.strip().split('\t')
        result=eval(result)
        r+=result[0]
        i+=result[1]
        s+=result[2]
        n+=result[3]
        num+=1
    print('替换：{} 插入:{} 删除:{} wer:{}  总字数:{} 总句子数:{}'.format(r,i,s,(r+i+s)/n,n,num))
