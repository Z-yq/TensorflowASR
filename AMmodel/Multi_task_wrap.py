...
class TrainingSampler(Sampler):
    ...
    def initialize(self):
        ...
        return (
            ...
            tf.tile([[tf.cast(self.config.startid,tf.float32)]], [self._batch_size, 768]),
        )
...
class DecoderCell(tf.keras.layers.AbstractRNNCell):
    ...
    def __init__(self,
        ...
        self.classes_projection = tf.keras.layers.Dense(
            units=768, name="classes_projection"
        )
...
class MultiTaskLAS(tf.keras.Model):
    ...
    def __init__(self, encoder,classes1,classes2,classes3, config, training, enable_tflite_convertible=False,speech_config=dict, **kwargs):
        ...
        self.fc_final = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(units=config.n_classes, activation="linear",
                                  use_bias=True), name="fully_connected4")
        ...
        self.token_project=tf.keras.Sequential([ConformerBlock(config.decoder_lstm_units,dropout=config.dropout,fc_factor=config.fc_factor,head_size=config.head_size,
                                          num_heads=config.num_heads,kernel_size=config.kernel_size,name='block%d'%i) for i in range(config.n_lstm_decoder+1)])
        self.config = config
    ...
    def call(
        ...
        encoder_hidden_states=tf.concat([encoder_hidden_states1, encoder_hidden_states2,encoder_hidden_states3],-1)
