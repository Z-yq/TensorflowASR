...
class BaseRunner():
    ...
    def __init__(self, config: dict):
        ...
        self.running_config["outdir"] = preprocess_paths(self.running_config["outdir"])
        # Writers
...
class BaseTrainer(BaseRunner):
    ...
    def set_strategy(self, strategy=None):
        ...
        self.global_batch_size = self.running_config["batch_size"] * self.strategy.num_replicas_in_sync
    ...
    def _check_log_interval(self):
        """Save log interval."""
        if self.steps % self.running_config["log_interval_steps"] == 0:
    ...
    def _check_save_interval(self):
        """Save log interval."""
        if self.steps % self.running_config["save_interval_steps"] == 0:
    ...
    def _check_eval_interval(self):
        """Save log interval."""
        if self.steps % self.running_config["eval_interval_steps"] == 0:
            self._eval_batches()
