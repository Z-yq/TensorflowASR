...
class CrossGlobalMHSAMoudle(ChunkMHSAModule):
    def call(self, inputs, key, mask=None, training=False, **kwargs):
        # outputs = self.pc(inputs)
        outputs = self.ln(inputs, training=training)
        outputs = self.mha(outputs, key, attention_mask=None, training=training)
        outputs = self.do(outputs, training=training)
        outputs = self.res_add([inputs, outputs])
        return outputs


class CrossChunkMHSAMoudle(ChunkMHSAModule):
    def init_cache(self, B):
        return tf.zeros([B, 0, self.d_model]), tf.zeros([B, 0, self.d_model])

    def call(self, inputs, key, mask=None, training=False, **kwargs):
        # outputs = self.pc(inputs)
        mask = self._compute_chunk_mask(inputs, self.win_front, self.win_back)
        outputs = self.ln(inputs, training=training)
        outputs = self.mha(outputs, key, attention_mask=mask, training=training)
        outputs = self.do(outputs, training=training)
        outputs = self.res_add([inputs, outputs])
        return outputs

    def stream_call(self, inputs, key, cache1, cache2, mask=None, training=False):
        T = tf.shape(inputs)[1]
        # print(T)
        new_cache_1 = tf.concat([cache1, inputs], axis=1)
        new_cache_2 = tf.concat([cache2, key], axis=1)
        outputs = self.ln(new_cache_1, training=training)
        mask = self._compute_chunk_mask(outputs, self.win_front, self.win_back)
        query = outputs[:, -T:]
        mask = mask[:, -T:]
        # tf.print('mha T:',T)
        outputs = self.mha(query, new_cache_2, training=training, attention_mask=mask, use_causal_mask=False)

        outputs = self.do(outputs, training=training)
        outputs = self.res_add([inputs, outputs])
        # new_cache=tf.concat([new_cache_1[tf.newaxis],new_cache_2[tf.newaxis]],axis=0)
        return outputs, new_cache_1, new_cache_2


class CrossChunkBlock(tf.keras.layers.Layer):
    def __init__(self, input_dim,
        ...
        self.mhsam = CrossChunkMHSAMoudle(
            head_size=head_size, num_heads=num_heads, dropout=dropout, win_front=win_front, win_back=win_back,
        )
    ...
    def init_cache(self, B):
        return self.mhsam.init_cache(B)

    # @tf.function(experimental_relax_shapes=True)
    def call(self, inputs, key, mask=None, training=False, **kwargs):
        outputs = self.ffm1(inputs, training=training)
        outputs = self.mhsam(outputs, key, mask=mask, training=training)

        outputs = self.ffm2(outputs, training=training)
        outputs = self.ln(outputs, training=training)
        return outputs

    def stream_call(self, inputs, key, cache1, cache2):
        outputs = self.ffm1(inputs, training=False)
        outputs, new_cache1, new_cache2 = self.mhsam.stream_call(outputs, key, cache1, cache2)

        outputs = self.ffm2(outputs, training=False)
        outputs = self.ln(outputs, training=False)
        return outputs, new_cache1, new_cache2
...
class CrossGlobalBlock(tf.keras.layers.Layer):
    def __init__(self, input_dim,
                 dropout=0.0,
                 fc_factor=0.5,
                 head_size=144,
        ...
        self.mhsam = CrossGlobalMHSAMoudle(
    ...
    def call(self, inputs, key, mask=None, training=False, **kwargs):
        outputs = self.ffm1(inputs, training=training)
        outputs = self.mhsam(inputs, key, mask=mask, training=training)
        outputs = self.ffm2(outputs, training=training)
        outputs = self.ln(outputs, training=training)
        return outputs
...
class CrossCorrectorA(tf.keras.Model):
    def __init__(
            self,
            num_classes,
            dmodel=144,
        ...
        super(CrossCorrectorA, self).__init__()
        self.decode_layers = []
        self.enc_layers = []
        self.cross_layers = []
        self.dmodel = dmodel
        ...
        for i in range(num_blocks):
            enc_block = ChunkConformerBlock(
                ...
                win_front=win_front,
                win_back=win_back,
                name=name + f"_enc_block_{i}",
            )
            self.enc_layers.append(enc_block)
        for i in range(num_blocks):
            cross_block = CrossChunkBlock(
                input_dim=dmodel,
                dropout=dropout,
                fc_factor=fc_factor,
                head_size=head_size,
                ...
                win_front=win_front,
                win_back=win_back,
                name=name + f"_cross_block_{i}",
            )
            self.cross_layers.append(cross_block)
        self.num_blocks = num_blocks
        self.fc = tf.keras.layers.Dense(
            units=num_classes,
            activation="linear",
    ...
    def _build(self):
        fake_enc = tf.random.uniform([1, 10, 144])
        fake_ctc = tf.random.uniform([1, 10, self.num_calsses])
        self(fake_ctc, fake_enc)

    def _compute_ctc_mask(self, ctc_output, blank):
        ctc_out = tf.argmax(ctc_output, -1)
        ctc_out = tf.expand_dims(ctc_out, -1)
        trm_mask_point = tf.where(ctc_out == blank, 1.0, 0.0)

        trm_mask_point_ = tf.transpose(trm_mask_point, [0, 2, 1])
        mask_matrix = tf.ones_like(trm_mask_point * trm_mask_point_)
        mask_matrix = trm_mask_point * mask_matrix + trm_mask_point_ * mask_matrix
        # causal_mask_matrix=self._compute_causal_mask(encoder_hidden_states)
        trm_mask = mask_matrix
        trm_mask = tf.clip_by_value(trm_mask, 0., 1.)
        trm_mask = tf.where(trm_mask == 1.0, 0., 1.)
        # trm_mask=tf.cast(trm_mask,tf.bool)
        return trm_mask

    def _compute_inp_mask(self, inputs):

        trm_mask_point = tf.where(inputs == 0, 1.0, 0.0)
    ...
    def init_caches(self, B):
        ...
        for layer in self.decode_layers:
            mha_cache, cnn_cache = layer.init_cache(B)
            new_dec_mha_caches.append(mha_cache[tf.newaxis])
            new_dec_cnn_caches.append(cnn_cache[tf.newaxis])
        new_dec_mha_caches = tf.concat(new_dec_mha_caches, axis=0)
        new_dec_cnn_caches = tf.concat(new_dec_cnn_caches, axis=0)

        new_enc_mha_caches = []
        new_enc_cnn_caches = []
        for layer in self.enc_layers:
            mha_cache, cnn_cache = layer.init_cache(B)
            new_enc_mha_caches.append(mha_cache[tf.newaxis])
            new_enc_cnn_caches.append(cnn_cache[tf.newaxis])
        new_enc_mha_caches = tf.concat(new_enc_mha_caches, axis=0)
        new_enc_cnn_caches = tf.concat(new_enc_cnn_caches, axis=0)

        new_cross_query_caches = []
        new_cross_key_caches = []
        for layer in self.cross_layers:
            cross_query_cache, corss_key_cache = layer.init_cache(B)
            new_cross_query_caches.append(cross_query_cache[tf.newaxis])
            new_cross_key_caches.append(corss_key_cache[tf.newaxis])
        new_cross_query_caches = tf.concat(new_cross_query_caches, axis=0)
        new_cross_key_caches = tf.concat(new_cross_key_caches, axis=0)
        new_mha_caches = tf.concat([new_dec_mha_caches[tf.newaxis], new_enc_mha_caches[tf.newaxis]], axis=0)
        new_cnn_caches = tf.concat([new_dec_cnn_caches[tf.newaxis], new_enc_cnn_caches[tf.newaxis]], axis=0)
        new_cross_caches = tf.concat([new_cross_query_caches[tf.newaxis], new_cross_key_caches[tf.newaxis]], axis=0)
        return new_mha_caches, new_cnn_caches, new_cross_caches

    def call(self, ctc_outs, enc_outs, training=False, mask=None):

        for i in range(self.num_blocks):
            dec_layer = self.decode_layers[i]
            enc_layer = self.enc_layers[i]
            cross_layer = self.cross_layers[i]
            inp_outs = self.project(ctc_outs, training=training)
            inp_outs = dec_layer(inp_outs, mask=mask, training=training)

            enc_outs = enc_layer(enc_outs, mask=mask, training=training)
            cross_outs = cross_layer(inp_outs, enc_outs)

            ctc_outs = self.fc(cross_outs, training=training)
        return ctc_outs

    def stream_call(self, ctc_outs, enc_outs, mha_caches, cnn_caches, cross_caches):
        ...
        for i in range(self.num_blocks):
            ...
            dec_mha_caches = mha_caches[0]
            dec_cnn_caches = cnn_caches[0]

            enc_mha_caches = mha_caches[1]
            enc_cnn_caches = cnn_caches[1]

            inp_outs = self.project(ctc_outs, training=training)

            dec_mha_cache = dec_mha_caches[i]
            dec_cnn_cache = dec_cnn_caches[i]
            inp_outs, new_dec_mha_cache, new_dec_cnn_cache = dec_layer.stream_call(inp_outs, dec_mha_cache,
                                                                                   dec_cnn_cache, training=training)

            new_dec_mha_caches.append(new_dec_mha_cache[tf.newaxis])
            new_dec_cnn_caches.append(new_dec_cnn_cache[tf.newaxis])

            enc_mha_cache = enc_mha_caches[i]
            enc_cnn_cache = enc_cnn_caches[i]
            enc_outs, new_enc_mha_cache, new_enc_cnn_cache = enc_layer.stream_call(enc_outs, enc_mha_cache,
                                                                                   enc_cnn_cache, training=training)

            new_enc_mha_caches.append(new_enc_mha_cache[tf.newaxis])
            new_enc_cnn_caches.append(new_enc_cnn_cache[tf.newaxis])

            cross_query_caches = cross_caches[0]
            cross_key_caches = cross_caches[1]

            cross_query_cache = cross_query_caches[i]
            cross_key_cache = cross_key_caches[i]
            cross_outs, new_cross_query_cache, new_cross_key_cahce = cross_layer.stream_call(inp_outs, enc_outs,
                                                                                             cross_query_cache,
                                                                                             cross_key_cache)

            new_cross_query_caches.append(new_cross_query_cache[tf.newaxis])
            new_cross_key_caches.append(new_cross_key_cahce[tf.newaxis])
        ...
        if self.win_back != 0:
            ...
            new_dec_mha_caches = new_dec_mha_caches[:, :, :-self.win_back]
            new_dec_cnn_caches = new_dec_cnn_caches[:, :, :-self.win_back]
            new_dec_mha_caches = new_dec_mha_caches[:, :, -self.win_front:]
            new_dec_cnn_caches = new_dec_cnn_caches[:, :, -self.kernel_size:]

            new_enc_mha_caches = new_enc_mha_caches[:, :, :-self.win_back]
            new_enc_cnn_caches = new_enc_cnn_caches[:, :, :-self.win_back]
            new_enc_mha_caches = new_enc_mha_caches[:, :, -self.win_front:]
            new_enc_cnn_caches = new_enc_cnn_caches[:, :, -self.kernel_size:]

            new_cross_query_caches = new_cross_query_caches[:, :, :-self.win_back]
            new_cross_key_caches = new_cross_key_caches[:, :, :-self.win_back]
            new_cross_query_caches = new_cross_query_caches[:, :, -self.win_front:]
            new_cross_key_caches = new_cross_key_caches[:, :, -self.win_front:]
        else:
            valid_outputs = ctc_outs
        ...
        new_cnn_caches = tf.concat([new_dec_cnn_caches[tf.newaxis], new_enc_cnn_caches[tf.newaxis]], axis=0)
        new_cross_caches = tf.concat([new_cross_query_caches[tf.newaxis], new_cross_key_caches[tf.newaxis]], axis=0)
        return valid_outputs, new_mha_caches, new_cnn_caches, new_cross_caches, unvalid_outputs
...
class ChunkConformer(tf.keras.Model):
    def __init__(self, config, phone, txt, *args, **kwargs):
        ...
        self.helper = CrossCorrectorB(**self.model_config['ContextHelper'])
